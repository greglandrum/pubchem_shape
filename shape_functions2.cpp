...
void getJointColorTypeSet(const unsigned *ref_atom_types,
  ...
  for (unsigned k = 0; k < fit_natom; ++k) {
    ...
    if (atom_type > 0) {
      if (ref_color_atom_type_set.find(atom_type) !=
          ref_color_atom_type_set.end()) {
...
void getVolumeAtomIndexVector(const unsigned *atom_types, const unsigned natom,
...
void getColorAtomType2IndexVectorMap(
    const unsigned *atom_types, const unsigned natom,
    std::map<unsigned, std::vector<unsigned>> &atom_type_to_index_vector_map) {
  for (unsigned k = 0; k < natom; ++k) {
    unsigned atom_type = atom_types[k];
    if (atom_type > 0) {
      std::map<unsigned, std::vector<unsigned>>::iterator ptr =
          atom_type_to_index_vector_map.find(atom_type);
...
void restrictColorAtomType2IndexVectorMap(
  ...
  for (std::map<unsigned, std::vector<unsigned>>::const_iterator iter =
           atom_type_to_index_vector_map.begin();
       iter != atom_type_to_index_vector_map.end(); ++iter) {
    unsigned atom_type = iter->first;
    if (joint_color_atom_type_set.find(atom_type) ==
        joint_color_atom_type_set.end()) {
...
void ComputeOverlapAndAnalyticGradient(
    ...
    const std::vector<unsigned> &fit_volume_atom_index_vector, bool needGrad) {
  ...
  for (unsigned i : fit_volume_atom_index_vector) {
    ...
    // Since we are applying the derivative on transformed coordinates,
...
double ComputeShapeOverlap(
    ...
    const std::vector<unsigned> &ref_volume_atom_index_vector,
    const float *fit_coord, const std::vector<double> &alpha_fit_vector,
  ...
  ComputeOverlapAndAnalyticGradient(
      overlap, NULL,
      ////   NULL,
      NULL, ref_coord, alpha_ref_vector, ref_volume_atom_index_vector,
      fit_coord, alpha_fit_vector, fit_volume_atom_index_vector,
      //////     gpparams,
      false);
...
double
ComputeFeatureOverlap(const float *ref_coord, // per ref atom, XYZ coordinates
                      const std::vector<double> &alpha_ref_vector,
                      ...
                      ) // Rot/trans matrix to rotate fit to ref
{
  ...
  if (debug_print2)
    std::cerr << "\n"
              << "^^^^ComputeFeatureOverlap : m=" << m << "\n";

  // Iteration over atom types
  for (std::map<unsigned, std::vector<unsigned>>::const_iterator ref_ptr =
           ref_atom_type_to_index_vector_map.begin();
       ref_ptr != ref_atom_type_to_index_vector_map.end(); ++ref_ptr) {
    const unsigned atom_type = ref_ptr->first;

    if (debug_print2)
      std::cerr << "^^^^ComputeFeatureOverlap : atom_type: " << atom_type
                << "\n";

    const std::vector<unsigned> &ref_index_vector = ref_ptr->second;
    std::map<unsigned, std::vector<unsigned>>::const_iterator fit_ptr =
        fit_atom_type_to_index_vector_map.find(atom_type);
...
double ComputeFeatureOverlap(const float *ref_coord,
                             const std::vector<double> &alpha_ref_vector,
                             const std::map<unsigned, std::vector<unsigned>>
                                 &ref_atom_type_to_index_vector_map,
  ...
  return ComputeFeatureOverlap(
      ref_coord, alpha_ref_vector, ref_atom_type_to_index_vector_map, fit_coord,
      alpha_fit_vector, fit_atom_type_to_index_vector_map, NULL);
}
...
double getD2CutOff() { return d2CutOff; }
...
void setAlpha(const double *rad, const unsigned natom,
              std::vector<double> &alpha_vec) {

  if (debug_print2)
    std::cerr << "--- natom: " << natom << "\n";
