...
void VRotTransMatrixNew(Uint8 &packed, const float *m) {
  packed = 0;
  ...
  if (0.1 < trace) { // Default
    ...
    //              ")"
    //              << "  (M: " << m[0] << " " << m[4] << " " << m[8] << ")" );
  } else if (0.005 < (m[0] - m[4]) && 0.005 < (m[0] - m[8])) { // X is largest
  ...
  if (rbase < p1) {
    //// //// PUBCHEM_WARN( "Qw encoding is corrupt (0-255 range)!  " << p1 << "
    ///" << qw * s );
  }
  if (rbase < p2) {
    //// //// PUBCHEM_WARN( "Qx encoding is corrupt (0-255 range)!  " << p2 << "
    ///" << qx * s );
  }
  if (rbase < p3) {
    //// //// PUBCHEM_WARN( "Qy encoding is corrupt (0-255 range)!  " << p3 << "
    ///" << qy * s );
  }
  if (rbase < p4) {
    //// //// PUBCHEM_WARN( "Qz encoding is corrupt (0-255 range)!  " << p4 << "
    ///" << qz * s );
  }

  // Encode Translation
  static const unsigned tbase =
      static_cast<unsigned>(pow(static_cast<double>(0x1FFFFFFF), (1.0 / 3.0)));
  ...
  if (p5 >= tbase) { // Range check
    p5 = tbase - 1;
    //// //// PUBCHEM_WARN( "X axis translation abs(" << m[  9 ] << ") is beyond
    ///maximum range of " << PubChem::Data::expPackedNew2[ p5 ] );
  }
  ...
  if (p6 >= tbase) { // Range check
    p6 = tbase - 1;
    //// //// PUBCHEM_WARN( "Y axis translation abs(" << m[ 10 ] << ") is beyond
    ///maximum range of " << PubChem::Data::expPackedNew2[ p6 ] );
  }
  ...
  if (p7 >= tbase) { // Range check
    p7 = tbase - 1;
    //// //// PUBCHEM_WARN( "Z axis translation abs(" << m[ 11 ] << ") is beyond
    ///maximum range of " << PubChem::Data::expPackedNew2[ p7 ] );
  }
...
void VQuaternionTransArray2RotTransMatrix(double *m, const double *q,
...
void VQuaternionTransArray2RotTransMatrix(float *m, const double *q,
                                          const bool normalize) {
  double q00 = q[0];
  double q01 = q[1];
  double q02 = q[2];
...
void VCombineRotTransMatrixMatrix(double *t, const double *t1,
                                  const double *t2) {
  // Rotation Matrix
...
void VRotTransMatrix2QuaternionTransArray(double *q, const double *m) {
  double trace = 1.0 + m[0] + m[4] + m[8];
  double s, qw, qx, qy, qz;
  if (trace > 0.00001) {
    s = 2.0 * sqrt(trace);
    ...
    qy = (m[2] - m[6]) * s_invert;
    qz = (m[3] - m[1]) * s_invert;
    ////// PUBCHEM_INFO( "0 trace: " << trace << "  " << qw << " " << qx << " "
    ///<< qy << " " << qz );
    ...
    qy = (m[3] + m[1]) * s_invert;
    qz = (m[2] + m[6]) * s_invert;
    ////// PUBCHEM_INFO( "1 trace: " << trace << "  " << qw << " " << qx << " "
    ///<< qy << " " << qz );
  } else if (m[4] > m[8]) {
    s = 2.0 * sqrt(1.0 + m[4] - m[0] - m[8]);
    double s_invert = 1.0 / s;
    ...
    qy = 0.25 * s;
    qz = (m[7] + m[5]) * s_invert;
    ////// PUBCHEM_INFO( "2 trace: " << trace << "  " << qw << " " << qx << " "
    ///<< qy << " " << qz );
  } else {
    s = 2.0 * sqrt(1.0 + m[8] - m[0] - m[4]);
    double s_invert = 1.0 / s;
    ...
    qy = (m[7] + m[5]) * s_invert;
    qz = 0.25 * s;
    ////// PUBCHEM_INFO( "3 trace: " << trace << "  " << qw << " " << qx << " "
    ///<< qy << " " << qz );
  }
                          ...
                          float *ev) // float[ 3 ]
{
  ...
  */
  ...
  // Diagonalize 3x3 steric inertial tensor matrix using Jacobi Diagonalization
  // method
  ...
  if (n > 1) {
    for (unsigned iter = 0; iter++ < max_iter;) {
      ...
      for (unsigned p = 0; p < 2; p++) {
        for (unsigned q = p + 1; q < 3; q++) {
          g = 100.0 * fabs(it[p][q]);
          if (4 < iter && g < convergence_threshold) {
    ...
    if (sum > convergence_threshold) {
      /////            PUBCHEM_WARN( "Convergence (" << sum << ") failed for
      ///inertial tensor diagonalization" );
    }
...
void VApplyRotTransMatrix(float *c, const unsigned n, const float *m) {
  return VApplyRotTransMatrix(c, c, n, m);
}
// Avoids need to copy array
void VApplyRotTransMatrix(float *c, const float *d, const unsigned n,
                          const float *m) {
  double m00 = m[0];
  double m01 = m[1];
  double m02 = m[2];
...
void TransformCoordToStericFrameAndCalculateQrat(
    const unsigned natom, const float threshold,
    float *coord, // float[ natom * 3 ]
    unsigned &qrat) {
...
unsigned CalculateQrat(const float *ev_ncbi, const float threshold) {
  ...
  /////  std::cerr << "** 1 ** double_EV_oe: " << double_ev_oe[0] << " " <<
  ///double_ev_oe[1] << " " << double_ev_oe[2] << "\n";

  std::sort(double_ev_oe, double_ev_oe + 3, std::greater<float>());

  ////  std::cerr << "** 2 ** double_EV_oe: " << double_ev_oe[0] << " " <<
  ///double_ev_oe[1] << " " << double_ev_oe[2] << "\n";
