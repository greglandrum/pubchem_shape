...
void Neighbor_Conformers(
    const float *ref_coord, const std::vector<double> &alpha_ref_vector,
    ...
    const double ref_sov, const double ref_sof, const float *fit_coord,
    ...
    const double fit_sov, const double fit_sof, const bool considerColorAtoms,
    const unsigned max_preiters, const unsigned max_postiters,
    const double opt_param, float *matrix, double &nbr_st, double &nbr_ct) {

  if (debug_print0) {
    std::cerr << "debug_print1: " << debug_print1 << "\n";
    std::cerr << "debug_print2: " << debug_print2 << "\n";
  }
  ...
  unsigned ref_qrat = 1000;
  {
    ...
    memcpy(&(ref_coord_work_vec[0]), ref_coord,
           (alpha_ref_vector.size() * 3 * sizeof(float)));
    TransformCoordToStericFrameAndCalculateQrat(
        alpha_ref_vector.size(), qrat_threshold, &(ref_coord_work_vec[0]),
        ref_qrat);
  }
  unsigned fit_qrat = 1000;
  {
    std::vector<float> fit_coord_work_vec(alpha_fit_vector.size() * 3);
    memcpy(&(fit_coord_work_vec[0]), fit_coord,
           (alpha_fit_vector.size() * 3 * sizeof(float)));
    TransformCoordToStericFrameAndCalculateQrat(
        alpha_fit_vector.size(), qrat_threshold, &(fit_coord_work_vec[0]),
        fit_qrat);
  }
  ...
  if (debug_print1)
    std::cerr << "*** st_sum_sov: " << st_sum_sov << "\n";
  if (debug_print1)
    std::cerr << "*** ct_sum_sov: " << ct_sum_sov << "\n";
  if (debug_print1)
    std::cerr << "*** st_ratio: " << st_ratio << "\n";
  if (debug_print1)
    std::cerr << "*** ct_ratio: " << ct_ratio << "\n";
  ...
  if (debug_print1)
    std::cerr << "^^^*** max_preiters: " << max_preiters << "\n";
  if (debug_print1)
    std::cerr << "^^^*** max_postiters: " << max_postiters << "\n";
  if (debug_print1)
    std::cerr << "^^^*** st_sum_sov: " << st_sum_sov << "\n";
  if (debug_print1)
    std::cerr << "^^^*** ct_sum_sov: " << ct_sum_sov << "\n";
  if (debug_print1)
    std::cerr << "^^^*** considerColorAtoms: " << considerColorAtoms << "\n";
  if (debug_print1)
    std::cerr << "*** npose: " << npose << "\n";
  if (debug_print1)
    std::cerr << "\n";
  ...
  for (unsigned ibest_pose = 0; ibest_pose < npose; ibest_pose++) {
    ...
    if (debug_print1)
      std::cerr << "Processing initial pose: " << ibest_pose
                << "\t is_qrat_nonzero: " << is_qrat_nonzero
                << "\t  npose: " << npose << "\n";

    // Point to appropriate transformation matrix for steric pose

    double qom[7]; // Grid-based pose 4x1 quaternion vector and 3x1 translation
                   // vector
    const double
        *mom; // Grid-based pose 3x3 rotation matrix and 3x1 translation vector
    double st_tot = (double)0; // AB volume overlap
    double g[7]; // Analytic gradient for 4x1 quaternion vector and 3x1
                 // translation vector (really 6x1 since [0] element is zero)

    if (debug_print1)
      std::cerr << "\tBefore qrat-based selection of poses, is_qrat_nonzero: "
                << is_qrat_nonzero << " st_tot: " << st_tot << "\n";

    if (is_qrat_nonzero) {

      if (debug_print1)
        std::cerr << "\t\tis_qrat_nonzero: " << is_qrat_nonzero
                  << " -- Select mom for ibest_pose, where ibest_pose="
                  << ibest_pose << "\n";

      memcpy(best_qom_pose[ibest_pose], qmatrix_typed[ibest_pose],
             7 * sizeof(double));
      memcpy(best_mom_pose[ibest_pose], matrix_typed[ibest_pose],
             12 * sizeof(double));
    } else {
      if (debug_print1)
        std::cerr << "\t\tis_qrat_nonzero: " << is_qrat_nonzero
                  << " -- Select 8 mom's for ibest_pose, where ibest_pose="
                  << ibest_pose << "\n";

      // Find best m_loc_ from mloc_ = 0, 1, ..., 6, set it to m_loc, and apply
      // insyed of the best pose from the grid
      unsigned m_loc = 0;
      double F = (double)-1;
      for (unsigned m_loc_ = 0; m_loc_ < 7; ++m_loc_) {
        ...
        ComputeOverlapAndAnalyticGradient(
            st_tot, g, mom, ref_coord, alpha_ref_vector,
            ref_volumeAtomIndexVector, fit_coord, alpha_fit_vector,
        ...
        if (considerColorAtoms) {
          if (ct_sum_sov > 0) {
            VQuaternionTransArray2RotTransMatrix(mc, qom,
                                                 false); // Already normalized
            ct_tot = ComputeFeatureOverlap(
                ref_coord, alpha_ref_vector, ref_colorAtomType2IndexVectorMap,
                fit_coord, alpha_fit_vector, fit_colorAtomType2IndexVectorMap,
                mc);
        ...
        if (debug_print1)
          std::cerr << "\t\tibest_pose: " << ibest_pose
                    << "\tm_loc_: " << m_loc_ << "\tst_tot: " << st_tot
                    << "\tst_sum_sov: " << st_sum_sov << "\tct_tot: " << ct_tot
                    << "\tct_sum_sov: " << ct_sum_sov << "\tfst: " << fst
                    << "\t fct: " << fct << "\t F_: " << F_ << "\n";
      ...
      if (debug_print1)
        std::cerr << "ibest_pose: " << ibest_pose << " FOUND m_loc: " << m_loc
                  << "\tF: " << F << "\n";

      // Setting "best" m_loc for the current best_pose ibest_pose
      memcpy(best_qom_pose[ibest_pose], pqmatrix_typed[ibest_pose][m_loc],
             7 * sizeof(double));
      memcpy(best_mom_pose[ibest_pose], pmatrix_typed[ibest_pose][m_loc],
             12 * sizeof(double));
    } // end if "qrat"
  }   // end loop ibest_pose

  const double max_quaternion_step = 0.075; // Maximum step size for quaternion
  const double max_translation_step =
      0.500; // Maximum step size for translation
  const double min_quaternion_step =
      0.0002; // Convergence criteria for quaternion
  const double min_translation_step =
      0.0020; // Convergence criteria for translation
  const double min_volume_overlap =
      0.0020; // Convergence criteria for ST AB volume overlap
  ...
  for (unsigned ibest_pose = 0; ibest_pose < npose; ibest_pose++) {
    ...
    qstep_size_pose[ibest_pose] =
        -0.0010; // Quaternion step size using gradient for direction
    tstep_size_pose[ibest_pose] =
        -0.0100; // Translation step size using gradient for direction

    // Evaluate the gradient at the starting point

    ComputeOverlapAndAnalyticGradient(
        st_tot_pose[ibest_pose], old_gradient_pose[ibest_pose],
        best_mom_pose[ibest_pose], ref_coord, alpha_ref_vector,
        ref_volumeAtomIndexVector, fit_coord, alpha_fit_vector,
        fit_volumeAtomIndexVector);

    if (debug_print1) {
      float fst =
          st_tot_pose[ibest_pose] / (st_sum_sov - st_tot_pose[ibest_pose]);
      std::cerr << "\tibest_pose: " << ibest_pose
                << " Beginning: st_tot: " << st_tot_pose[ibest_pose]
                << "; fst: " << fst << "\n";
    }
  }
  ...
  for (unsigned int icycle = 0; icycle < 2; ++icycle) {
    for (unsigned ibest_pose = 0; ibest_pose < npose; ibest_pose++) {
      ...
      if (icycle == 1) { // Beginning of the cycle with icycle=0
        ...
        if (debug_print1)
          std::cerr << "*** icycle: " << icycle << " ibest_pose: " << ibest_pose
                    << " nbr_comb_prelim: " << nbr_comb_prelim
                    << " fcomb_best_pose[ ibest_pose ]: "
                    << fcomb_best_pose[ibest_pose]
                    << " fcomb_best_pose[ ibest_pose ]/nbr_comb_prelim: "
                    << fcomb_best_pose[ibest_pose] / nbr_comb_prelim << "\n";
        ...
        if (debug_print1) {
          if (allowed_pose[ibest_pose]) {
            std::cerr << "*** pose: " << ibest_pose << " allowed"
                      << "\n";
          } else {
            std::cerr << "*** pose: " << ibest_pose << " not allowed"
                      << "\n";
          }
        }
      }
      ...
      if (allowed_pose[ibest_pose]) {
        if (debug_print1)
          std::cerr << "\tibest_pose: " << ibest_pose
                    << " Compute (non-grid-based) shape volume overlap for the "
                       "current pose ibest_pose "
                    << ibest_pose << " and its \"best\" starting qom"
                    << "\n";
        ...
        for (unsigned iter = start_iter; iter < end_iter; iter++) {
          ...
          double quat_squared; // Scratch for deriving fourth quaternion value
                               // [0] from other three [1-3]
          double step[7];      // Actual step taken
          //                    unsigned  ustep[ 6 ];  // Standard step size

          // If not first iteration and if we bracketed the maximum, use old
          // step
          ...
          if (0 == iter) {
            ...
            if (std::signbit(g[1]) != std::signbit(old_gradient[1])) {
              step[1] = (((qom[1] * fabs(old_gradient[1])) +
                          (old_quattrans[1] * fabs(g[1]))) /
                         (fabs(old_gradient[1]) + fabs(g[1]))) -
                        qom[1];
            ...
            if (std::signbit(g[2]) != std::signbit(old_gradient[2])) {
              step[2] = (((qom[2] * fabs(old_gradient[2])) +
                          (old_quattrans[2] * fabs(g[2]))) /
                         (fabs(old_gradient[2]) + fabs(g[2]))) -
                        qom[2];
            ...
            if (std::signbit(g[3]) != std::signbit(old_gradient[3])) {
              step[3] = (((qom[3] * fabs(old_gradient[3])) +
                          (old_quattrans[3] * fabs(g[3]))) /
                         (fabs(old_gradient[3]) + fabs(g[3]))) -
                        qom[3];
            ...
            if (std::signbit(g[4]) != std::signbit(old_gradient[4])) {
              step[4] = (((qom[4] * fabs(old_gradient[4])) +
                          (old_quattrans[4] * fabs(g[4]))) /
                         (fabs(old_gradient[4]) + fabs(g[4]))) -
                        qom[4];
            ...
            if (std::signbit(g[5]) != std::signbit(old_gradient[5])) {
              step[5] = (((qom[5] * fabs(old_gradient[5])) +
                          (old_quattrans[5] * fabs(g[5]))) /
                         (fabs(old_gradient[5]) + fabs(g[5]))) -
                        qom[5];
            ...
            if (std::signbit(g[6]) != std::signbit(old_gradient[6])) {
              step[6] = (((qom[6] * fabs(old_gradient[6])) +
                          (old_quattrans[6] * fabs(g[6]))) /
                         (fabs(old_gradient[6]) + fabs(g[6]))) -
                        qom[6];
          ...
          // Retain last step information in case we are not happy with "line
          // search"
          ...
          if (debug_print1)
            std::cerr << "\t\tibest_pose: " << ibest_pose << " iter: " << iter
          ...
          for (unsigned line_iter = 0; false == converged; line_iter++) {
            ...
            // reasonable size
            double mtstep =
                fmax(fmax(fabs(step[4]), fabs(step[5])), fabs(step[6]));
            if (mtstep > max_translation_step) {
              ...
              if (debug_print1)
                std::cerr << "\t\t\tibest_pose: " << ibest_pose
                          << " iter: " << iter << "\tline_iter: " << line_iter
                          << " too large max step; applying scaling factor "
                             "tscale_factor: "
                          << tscale_factor << "\n";

              if (fabs(step[4]) > max_translation_step)
                step[4] *= tscale_factor;
              if (fabs(step[5]) > max_translation_step)
                step[5] *= tscale_factor;
              if (fabs(step[6]) > max_translation_step)
                step[6] *= tscale_factor;
            }

            double mqstep =
                fmax(fmax(fabs(step[1]), fabs(step[2])), fabs(step[3]));
            if (mqstep > max_quaternion_step) {
              ...
              if (fabs(step[1]) > max_quaternion_step)
                step[1] *= qscale_factor;
              if (fabs(step[2]) > max_quaternion_step)
                step[2] *= qscale_factor;
              if (fabs(step[3]) > max_quaternion_step)
                step[3] *= qscale_factor;
            }

            if (min_quaternion_step > mqstep &&
                min_translation_step >
                    mtstep) { // Did we converge (so to speak)?
              st_tot = 0.0; // We didn't compute the new overlap yet, ensure we
                            // use the old one.
              converged = true;
              ...
              if (debug_print1)
                std::cerr << "\t\t\tibest_pose: " << ibest_pose
                          ...
                          << " " << min_quaternion_step << " " << mqstep << " "
                          << min_translation_step << " " << mtstep << ". BREAK "
                          << "\n";
              break;
            } else {
              if (debug_print1)
                std::cerr << "\t\t\tibest_pose: " << ibest_pose
                          << " iter: " << iter << " line_iter: " << line_iter
                          << "-- line search is not converged yet because "
                          << min_quaternion_step << " " << mqstep << " "
                          << min_translation_step << " " << mtstep << "\n";
            }
            ...
            // Update quaternion with step size (getting new rot/trans matrix to
            // use in the process)

            double mc[12], qc[7], tm1[12],
                tm2[12]; // Scratch array for rot/trans matrix

            // Functions originally from PubChem::Data
            VQuaternionTransArray2RotTransMatrix(tm1, qom,
                                                 false); // Already normalized
            VQuaternionTransArray2RotTransMatrix(tm2, step,
                                                 false); // Already normalized
            VCombineRotTransMatrixMatrix(mc, tm2, tm1);  // New rot/trans matrix
            VRotTransMatrix2QuaternionTransArray(qc, mc); // New quaternion

            // QC is not used in below function since always assumed to be
            // (1,0,0,0) ... need to remove it...

            ComputeOverlapAndAnalyticGradient(
                st_tot, g, mc, ref_coord, alpha_ref_vector,
                ref_volumeAtomIndexVector, fit_coord, alpha_fit_vector,
                fit_volumeAtomIndexVector);
            ...
            if (st_tot > old_overlap) {
              ...
              if (debug_print1)
                std::cerr << "\t\t\tibest_pose: " << ibest_pose
                          << " iter: " << iter << " line_iter: " << line_iter
                          << "-- we made a good step step because st_tot "
                             "increased. old_overlap: "
                          << old_overlap << " st_tot: " << st_tot << ". BREAK"
                          << "\n";
              ...
              if (debug_print1)
                std::cerr << "\t\t\tibest_pose: " << ibest_pose
                          << " iter: " << iter << " line_iter: " << line_iter
                          << "-- we DID NOT make a good step step because "
                             "st_tot did not increase. old_overlap: "
                          << old_overlap << " st_tot: " << st_tot << "\n";
            }
            ...
            if (2 == line_iter) {
              ...
              if (debug_print1)
                std::cerr
                    << "\t\t\tibest_pose: " << ibest_pose << " iter: " << iter
                    << " line_iter: " << line_iter
                    << "-- we line_iter=2 and we did not make a good step step "
                       "because st_tot did not increase. Decrease steps."
                    << "\n";
              ...
              if (debug_print1)
                std::cerr
                    << "\t\t\tibest_pose: " << ibest_pose << " iter: " << iter
                    << " line_iter: " << line_iter
                    << "-- we line_iter>3 and we did not make a good step step "
                       "because st_tot did not increase. Set converged. BREAK."
                    << "\n";
              ...
              if (debug_print1)
                std::cerr
                    << "\t\t\tibest_pose: " << ibest_pose << " iter: " << iter
                    << " line_iter: " << line_iter
                    << "-- we line_iter = 1,3 and we did not make a good step "
                       "step because st_tot did not increase. Changed steps."
                    << "\n";

              if (std::signbit(g[1]) != std::signbit(old_gradient[1])) {
                step[1] = (((qc[1] * fabs(old_gradient[1])) +
                            (old_quattrans[1] * fabs(g[1]))) /
                           (fabs(old_gradient[1]) + fabs(g[1]))) -
                          qc[1];
                double new_step = qstep_size * g[1];
                ...
                step[1] = qstep_size * g[1];
                         ...
                         fabs(old_gradient[1])) { // Going wrong way (relative
                                                  // to other components)?
                step[1] += qstep_size * g[1];
              } else {
              ...
              if (std::signbit(g[2]) != std::signbit(old_gradient[2])) {
                step[2] = (((qc[2] * fabs(old_gradient[2])) +
                            (old_quattrans[2] * fabs(g[2]))) /
                           (fabs(old_gradient[2]) + fabs(g[2]))) -
                          qc[2];
                double new_step = qstep_size * g[2];
                ...
                step[2] = qstep_size * g[2];
                         ...
                         fabs(old_gradient[2])) { // Going wrong way (relative
                                                  // to other components)?
                step[2] += qstep_size * g[2];
              } else {
              ...
              if (std::signbit(g[3]) != std::signbit(old_gradient[3])) {
                step[3] = (((qc[3] * fabs(old_gradient[3])) +
                            (old_quattrans[3] * fabs(g[3]))) /
                           (fabs(old_gradient[3]) + fabs(g[3]))) -
                          qc[3];
                double new_step = qstep_size * g[3];
                ...
                step[3] = qstep_size * g[3];
                         ...
                         fabs(old_gradient[3])) { // Going wrong way (relative
                                                  // to other components)?
                step[3] += qstep_size * g[3];
              } else {
              ...
              if (std::signbit(g[4]) != std::signbit(old_gradient[4])) {
                step[4] = (((qc[4] * fabs(old_gradient[4])) +
                            (old_quattrans[4] * fabs(g[4]))) /
                           (fabs(old_gradient[4]) + fabs(g[4]))) -
                          qc[4];
                ...
                step[4] = qstep_size * g[4];
                         ...
                         fabs(old_gradient[4])) { // Going wrong way (relative
                                                  // to other components)?
                step[4] += qstep_size * g[4];
              } else {
              ...
              if (std::signbit(g[5]) != std::signbit(old_gradient[5])) {
                step[5] = (((qc[5] * fabs(old_gradient[5])) +
                            (old_quattrans[5] * fabs(g[5]))) /
                           (fabs(old_gradient[5]) + fabs(g[5]))) -
                          qc[5];
                ...
                step[5] = qstep_size * g[5];
                         ...
                         fabs(old_gradient[5])) { // Going wrong way (relative
                                                  // to other components)?
                step[5] += qstep_size * g[5];
              } else {
              ...
              if (std::signbit(g[6]) != std::signbit(old_gradient[6])) {
                step[6] = (((qc[6] * fabs(old_gradient[6])) +
                            (old_quattrans[6] * fabs(g[6]))) /
                           (fabs(old_gradient[6]) + fabs(g[6]))) -
                          qc[6];
                ...
                step[6] = qstep_size * g[6];
                         ...
                         fabs(old_gradient[6])) { // Going wrong way (relative
                                                  // to other components)?
                step[6] += qstep_size * g[6];
              } else {
          ...
          ///// if( //// DebugOptions::debug_print1 ) std::cerr << "***** End of
          /// line search. converged: " << converged << " min_volume_overlap > (
          /// st_tot - old_overlap ) : " << ( min_volume_overlap > ( st_tot -
          /// old_overlap ) ) << "\n";

          // Did we converge?
          if (true == converged ||
              min_volume_overlap > (st_tot - old_overlap)) {
            if (debug_print1)
              std::cerr << "\t\tibest_pose_pose: " << ibest_pose
                        << " iter: " << iter
                        << " converged. converged: " << converged
                        << " min_volume_overlap: " << min_volume_overlap
                        << " st_tot - old_overlap: " << (st_tot - old_overlap)
                        << " WILL BREAK"
                        << "\n";

            if (old_overlap > st_tot) { // Previous successful step was
                                        // better... retain better step
              st_tot = old_overlap;
              memcpy(qom, old_quattrans, 7 * sizeof(float));

              if (debug_print1)
                std::cerr << "\t\titer: " << iter
                          << " keep previous value of st_tot. ";
            } else {
              if (debug_print1)
                std::cerr << "\t\titer: " << iter
                          << " keep previous value of st_tot. ";
        ...
        if (debug_print1)
          std::cerr
              ...
              << " End of optimization iterations for ibest_pose. st_tot: "
              << st_tot << "\tfst: " << fst << "\t nbr_st: " << nbr_st
              << "\tnbr_ct: " << nbr_ct << "\tfst: " << fst << "\tfct: " << fct
              << "\n";
  ...
  if (debug_print1)
    std::cerr << "FINAL RESULT: fst: " << nbr_st << " fct: " << nbr_ct
              << " fcomb: " << nbr_comb << "\n";
}
